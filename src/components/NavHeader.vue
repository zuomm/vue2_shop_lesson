...
export default {
  ...
  computed:{
      ...mapState(['nickName','cartCount']) //解构赋值
  },
  mounted(){
    this.checkLogin(); //在NavHeader组件挂载的时候就检测是否登录了
  },
  methods:{
      checkLogin(){
        axios.get("/users/checkLogin").then((response)=>{
            ...
            if(res.status=="0"){
                // this.nickName = res.result; 用vuex管理
